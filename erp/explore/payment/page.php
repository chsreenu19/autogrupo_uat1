	...
	<div class="lesting-area bg-area">
		<div class="container-fluid">
			...
			<div class="row grid_view" id="view_change">
				...
				$statement = $pdo->prepare("SELECT * FROM tbl_car WHERE status=1 AND (car_condition=? or car_condition=?)");
				$statement->execute(array('Used Car','Rentals'));
				$total_pages = $statement->rowCount();
				...
				$statement = $pdo->prepare("SELECT * FROM tbl_car WHERE status=1 AND (car_condition=? or car_condition=?) ORDER BY car_id DESC LIMIT $start, $limit");
				$statement->execute(array('Used Car','Rentals'));
				$result = $statement->fetchAll(PDO::FETCH_ASSOC);
				...
				{   
					$pagination .= "<div class=\"pagination\">";
					if ($page > 1) 
						$pagination.= "<a href=\"".BASE_URL."page.php?slug=used-cars&page="."$prev\">&#171; previous</a>";
					else
						$pagination.= "<span class=\"disabled\">&#171; previous</span>";    
        if ($lastpage < 7 + ($adjacents * 2))   //not enough pages to bother breaking it up
        {   
        	for ($counter = 1; $counter <= $lastpage; $counter++)
        	{
        		if ($counter == $page)
        			...
        			$pagination.= "<a href=\"".BASE_URL."page.php?slug=used-cars&page="."$counter\">$counter</a>";                 
        	}
        }
        elseif($lastpage > 5 + ($adjacents * 2))    //enough pages to hide some
        {
        	if($page < 1 + ($adjacents * 2))        
        	{
        		...
        		$pagination.= "...";
        		$pagination.= "<a href=\"".BASE_URL."page.php?slug=used-cars&page="."$lpm1\">$lpm1</a>";
        		$pagination.= "<a href=\"".BASE_URL."page.php?slug=used-cars&page="."$lastpage\">$lastpage</a>";       
        	}
        	elseif($lastpage - ($adjacents * 2) > $page && $page > ($adjacents * 2))
        	{
        		$pagination.= "<a href=\"".BASE_URL."page.php?slug=used-cars&page="."1\">1</a>";
        		$pagination.= "<a href=\"".BASE_URL."page.php?slug=used-cars&page="."2\">2</a>";
        		$pagination.= "...";
        		for ($counter = $page - $adjacents; $counter <= $page + $adjacents; $counter++)
        		{
        			if ($counter == $page)
        ...
        if ($page < $counter - 1) 
        	$pagination.= "<a href=\"".BASE_URL."page.php?slug=used-cars&page="."$next\">next &#187;</a>";
        else
        	$pagination.= "<span class=\"disabled\">next &#187;</span>";
        $pagination.= "</div>\n";       
    }
