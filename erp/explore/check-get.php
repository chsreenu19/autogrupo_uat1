...
if(!empty($_GET['year_id'])) {
	if($_GET['year_id'] == 'All Years') {
		$a_syear = '';
		$b_syear = '';
	} else {
		$a_syear = '&year=';
		$b_syear = $_GET['year_id'];
	}	
} else {
	$a_syear = '';
	$b_syear = '';
...
if(!empty($_GET['to_year_id'])) {
	if($_GET['to_year_id'] == 'All Years') {
		$a_to_year_id = '';
		$b_to_year_id = '';
	} else {
		$a_to_year_id = '&to_year=';
		$b_to_year_id = $_GET['to_year_id'];
	}	
} else {
	$a_to_year_id = '';
	$b_to_year_id = '';
...
if(!empty($_GET['vinnumber'])) {
	if($_GET['vinnumber'] == 'All VIN') {
		$a_vin = '';
		$b_vin = '';
	} else {
		$a_vin = '&vinnumber=';
		$b_vin = $_GET['vinnumber'];
	}	
} else {
	$a_vin = '';
	$b_vin = '';
...
if(!empty($_GET['productphotos'])) {
	if($_GET['productphotos'] == 'All VIN') {
		$a_productphotos = '';
		$b_productphotos = '';
	} else {
		$a_productphotos = '&productphotos=';
		$b_productphotos = $_GET['productphotos'];
	}	
} else {
	$a_productphotos = '';
	$b_productphotos = '';
...
//echo "<pre>".$a_productphotos.$b_productphotos; print_r($_GET);// exit;

$all_gets = '';
$all_gets = $a_brand.$b_brand.$a_syear.$b_syear.$a_to_year_id.$b_to_year_id.$a_vin.$a_productphotos.$b_productphotos.$b_vin.$a_model.$b_model.$a_car_condition.$b_car_condition.$a_price_range.$b_price_range.$a_car_category_id.$b_car_category_id.$a_body_type_id.$b_body_type_id.$a_fuel_type_id.$b_fuel_type_id.$a_transmission_type_id.$b_transmission_type_id.$a_year.$b_year.$a_mileage_start.$b_mileage_start.$a_mileage_end.$b_mileage_end.$a_country.$b_country.$a_state.$b_state.$a_city.$b_city;
//echo $all_gets; exit;
if($all_gets != '') {
