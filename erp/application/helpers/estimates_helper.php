...
//ableit for journals posting
function getdealrecapitemsforjournals($estimateId,$leadid, $staffid){
    $CI = &get_instance();
    $returnarray = array();
    $tblitemsquery = "SELECT * FROM ".db_prefix()."itemable WHERE rel_id=".$estimateId." AND rel_type='estimate' AND inv_id>0";
    $tblitemsexe = $CI->db->query($tblitemsquery);
    $tblitemsreult = $tblitemsexe->result_array();
    
    $returnarray['jnlvin'] =  $tblitemsreult;
	if(count($tblitemsreult)>0){
        $podetails = $CI->db->query("SELECT a.pur_order, a.unit_price,	b.order_date, b.pur_order_number, c.sku_code, c.id as 'inv_id' FROM ".db_prefix()."pur_order_detail a LEFT JOIN ".db_prefix()."pur_orders b ON a.pur_order = b.id LEFT JOIN ".db_prefix()."items c on a.item_code=c.id WHERE item_code =".$tblitemsreult[0]['inv_id'])->result_array();
       
        $returnarray['podetails'] =  $podetails;
    }else{
        $returnarray['podetails'] =  array();
    }
    $returnarray['jnlstaff'][0] =(array) get_staff($staffid);

    $leadquery = "SELECT * FROM ".db_prefix()."leads WHERE id=".$leadid;
    $leadexe = $CI->db->query($leadquery);
    $leadresult = $leadexe->result_array();
    $returnarray['jnllead'] = $leadresult;

    $quotequery = "SELECT * FROM ".db_prefix()."proposals WHERE estimate_id=".$estimateId;
    $quoteexe = $CI->db->query($quotequery);
    $quoteresult = $quoteexe->result_array();
    $returnarray['jnlproposal'] = $quoteresult;

    return $returnarray;

}

function getinventorydetailsbyid($id){
    $CI = &get_instance();
    $tblitemsquery = "SELECT * FROM ".db_prefix()."items WHERE id=".$id;
    $tblitemsexe = $CI->db->query($tblitemsquery);
    $tblitemsreult = $tblitemsexe->result_array();
    return $tblitemsreult;
}

//ableit for journalss posting