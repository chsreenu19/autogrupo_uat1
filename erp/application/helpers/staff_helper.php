...
//ableit tech get user role id
function get_staff_role_id_value()
{
    return $GLOBALS['current_user']->role;
}

function get_staff_role_id_restrict_forms()
{
    $role = $GLOBALS['current_user'];
    $roleId = $role->role;
    
    $formspermissionsroles = json_decode(QUOTEFORMS_ALLOWED_ROLES,TRUE);
    //echo "<pre>"; print_r($formspermissionsroles); exit;
    if(!in_array($roleId, $formspermissionsroles)){
        redirect(404);
    }
