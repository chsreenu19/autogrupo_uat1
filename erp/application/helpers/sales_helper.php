...
if (!function_exists('format_organization_info_ableit')) {
    /**
     * Format company info/address format
     * @return string
     */
    function format_organization_info_ableit()
    {
        $format = get_option('company_info_format');
        $vat    = get_option('company_vat');
		...
		$orgArray = array();
		$orgArray['company_name']=get_option('invoice_company_name');
		$orgArray['address']=get_option('invoice_company_address');
		$orgArray['city']=get_option('invoice_company_city');
		$orgArray['state']=get_option('company_state');
		$orgArray['zip_code']=get_option('invoice_company_postal_code');
		$orgArray['country_code']=get_option('invoice_company_country_code');
		$orgArray['phone']=get_option('invoice_company_phonenumber');
		
		$custom_company_fields = get_company_custom_fields();

        foreach ($custom_company_fields as $field) {
            //$format = _info_format_custom_field($field['id'], $field['label'], $field['value'], $format);
			$orgArray[$field['label']] = $field['value'];
        }
    ...
    if($str!=''){
        if (strpos($str, '-vin-') !== false) {
            ...
            if(is_array($explodeStr) && count($explodeStr)>0){
                ...
                $pitemsresults = $CI->db->query($pitems);
...
//ableit
//added by ableit
function get_items_table_data_able($transaction, $type, $for = 'html', $admin_preview = false)
{
	//echo 'here'; exit;
    include_once(APPPATH . 'libraries/App_items_table_able.php');
    $class = new App_items_table_able($transaction, $type, $for, $admin_preview);
