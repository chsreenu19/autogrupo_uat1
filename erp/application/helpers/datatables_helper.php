...
function render_datatable_commissionsreport($headings = [], $class = '', $additional_classes = [''], $table_attributes = [])
{
    $_additional_classes = '';
    $_table_attributes   = ' ';
    if (count($additional_classes) > 0) {
    ...
    foreach ($headings as $heading) {
        ...
        if (!is_array($heading)) {
            if($heading=='Sales Cost' || $heading == 'Purchase Cost' || $heading=='Margin' || $heading=='Commission'){
                $table .= '<th>' . $heading . '</th>';
            }else{
                $table .= '<th></th>';
            }
            ...
            $th_attrs = ($th_attrs != '' ? ' ' . $th_attrs : $th_attrs);
            if($heading['name']=='Sales Cost' || $heading['name'] == 'Purchase Cost' || $heading['name']=='Margin'){
                $table .= '<th' . $th_attrs . '>eee' . $heading['name'] . '</th>';
            }else{
                $table .= '<th' . $th_attrs . '>eee</th>';
            }
