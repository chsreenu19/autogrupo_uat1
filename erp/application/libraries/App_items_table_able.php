...
class App_items_table_able extends App_items_table_template
{
    ...
    public function items()
    {
        ...
        foreach ($this->items as $item) {
			...
			if (!empty($item['long_description'])) {
				//$arr = explode("-Inv-", $item['long_description']);
				//print_r($arr[1]); exit;
				if (strpos($item['long_description'], "Used Car") !== false) {
					$a = $item['id'];
				}else{
					//$a = 'javascript:void(0);';
					$a = $item['id'];
			...
			if (!empty($item['description'])) {
				...
				if (strpos($item['description'], " -vin- ") !== false) {
					$arr = explode(" -vin- ", $item['description']);
					//print_r($arr); exit;
					$vinid = $arr[1];
					$desc = $arr[0];
				}else{
            ...
            if (!empty($vinid)) {
                $itemHTML .= '<span style="font-size:' . $this->get_pdf_font_size() . 'px;"><a target="blank" href="'.base_url().'proposal/warranty/'.$vinid.'"><i class="fa fa-print" aria-hidden="true"></i>&nbsp; &nbsp;<strong>'
                . $this->period_merge_field($desc)
                . '</strong></a></span>';
