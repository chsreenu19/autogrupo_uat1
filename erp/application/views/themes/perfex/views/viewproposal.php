...
<div id="proposal-wrapper">
    <?php
      ob_start();
      $items = get_items_table_data_able($proposal, 'proposal')
      ->add_table_class('no-margin')
      ->set_headings('estimate');

     echo $items->table();
	  $staffimg = staff_profile_image($proposal->assigned,array('img','img-responsive','staff-profile-image-thumb'),'thumb');

	  $staffDts = get_staff($proposal->assigned);
    ...
    <div class="row">
        ...
        <div class="col-md-4 proposal-right">
          ...
          echo ' active';
						...
						<?php 
						  if(is_object($staffDts) && isset($staffDts->staffid)){
						  ?>
										  
						  <div class="row mtop20">
							
							 <!--<div class="col-md-5 text-muted proposal-status">
							   <?php echo $staffimg; ?>
							 </div>-->
							 <div class="col-md-7 proposal-status">
							  <h4>Assined To</h4>
								<?php echo $staffDts->full_name. '<br/>'.$staffDts->email. '<br/>'.$staffDts->phonenumber; ?>
							 </div>
						  </div>
