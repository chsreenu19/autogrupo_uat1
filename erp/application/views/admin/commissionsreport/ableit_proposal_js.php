...
//alert(1);
	...
	$("<div class='row tohideifnottradein tradeinhr'><div class='col-md-12 text-center'><hr class=''/><h4>Trade In Details </h4></div></div>").insertAfter( $('form select[name="custom_fields[proposal][89]" ').closest(".col-md-4") );
	...
	$("body").on("blur",'input[name="custom_fields[proposal][61]"]',function(){		
		...
		if(tradein !=''){
			...
			/*setTimeout(function () {	
			$('textarea[name="description"]').val('Balance');
			    $('textarea[name="long_description"]').val('Balance (Tradein Allowance - Bank payoff)');
			    $('input[name="rate"]').val(upval);				
				setTimeout(function () {	
					$('textarea[name="description"]').closest('tr').find('button').click();
					$('.additemtotable').click();	
					//$('input[name="custom_fields[proposal][61]"]').click().blur();
					alert_float("success", 'Added Balance as line item');	
				}, 300);
			}, 2000);*/
	...
	var arr = [
				...
				//'input[name="custom_fields[proposal][76]"]',
		...
		$.each(arr , function(index, val) { 
			...
			$(val).closest(".col-md-2,.col-md-3,.col-md-4").hide();
		});	
	...
	$("body").on("blur",'input[name="custom_fields[proposal][52]"]',function(){
		...
		if($(this).val()!=''){
			alert_float("info", 'Please wait, fetching VIN details');
        $.ajax({
            ...
            success: function(result)
            {
				...
				if(result.spec.success==true){	
					alert_float("success", 'Please wait, updating VIN details');
