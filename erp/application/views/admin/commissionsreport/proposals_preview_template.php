                     ...
                     <a target="blank" style="background: yellow; color: red;" href="<?php echo admin_url(); ?>creditapplication/client/<?php echo $proposal->id ?>" class="creditapplication" id="<?php echo $proposal->id; ?>">Credit Appliation</a>
						...
						<li>
                            ...
                            <?php } else if($status==SOLD_INVOICE_PENDING_ID && $proposal->status==3){ ?>
                                ...
                                <a id="soldInvoicePending" href="<?php echo admin_url() . 'proposals/mark_action_status/' . $status . '/' . $proposal->id; ?>"><?php echo _l('proposal_mark_as', format_proposal_status($status, '', false)); ?></a>
...
$(document).ready(function(){
	$("body").on("click",".esignature",function(){
		var id = $(this).attr("id");
        alert_float('info', "Please wait, while we validate the required data");
		$.ajax({
			type:'get',
               ...
               //debugger;
				if((typeof json.Token != 'undefined')){
					...
					/*if(json.Data.ErrorMessage !=''){
						alert_float('danger', json.Data.ErrorMessage); 
					}*/
				}else if(json.Data.ErrorMessage !=''){
                    alert_float('danger', JSON.stringify(json.Data.ErrorMessage));  
                }
	...
	$("body").on("click",".transferforms",function(){
		...
		var urlList = "<?php echo base_url(); ?>/inv/rve/"+$(this).attr('id')+", <?php echo base_url(); ?>/inv/mvsl/"+$(this).attr('id')+" ";
		//alert(urlList);
