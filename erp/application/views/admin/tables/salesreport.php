...
foreach ($rResult as $aRow) {
	...
	}elseif($aRow['tblcustomfields.slug']=='proposal_life_insurance'){
        $bgcs = get_batter_guard_cost($aRow['tblproposals.id']);
        $row[] = app_format_money($bgcs, ($aRow['currency'] != 0 ? get_currency(1) : $baseCurrency));
    }else{
	...
	if($aRow['tblcustomfields.slug']=='proposal_gap_policy'){
        ...
        if($aRow['tblcustomfieldsvalues.value']>0){
            $margin = $aRow['tblcustomfieldsvalues.value'] - get_batter_guard_cost($aRow['tblproposals.id']);
        }else{
            $margin = 0;
        }
