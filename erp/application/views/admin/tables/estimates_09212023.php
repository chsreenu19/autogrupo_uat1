...
foreach ($rResult as $aRow) {
   ...
   // $row[] = app_format_money($aRow['total_tax'], $aRow['currency_name']);
    ...
    $getProposalIdByESTIDQry = "SELECT 
        ...
        if(is_array($getProposalIdByESTIDResult) && count($getProposalIdByESTIDResult)>0){
            ...
            $bQuery = "SELECT a.id,
                        ...
                        WHERE a.id IN(". $bids.") AND relid=".$getProposalIdByESTIDResult[0]['quoteId'];
           ...
           if(is_array($bQueryResult) && count($bQueryResult)>0){
             ...
             foreach($bQueryResult as $bsRow){
                ...
                if($bsRow['slug']=='proposal_service_contract'){                    
                    $bsPrice += 379;
                    $bsCost += 174;                                   
                }

                if($bsRow['slug']=='proposal_credit_total_2'){                    
                    $bsPrice += 995;
                    $bsCost += 205;                                   
                }
    ...
    if(is_array($getProposalIdByESTIDResult) && count($getProposalIdByESTIDResult)>0){
        ...
        $insuranceQry = "select `value` from tblcustomfieldsvalues where fieldid IN( ".$Ibids. ") and relid=".$getProposalIdByESTIDResult[0]['quoteId'];
