            ...
            <div class="col-md-6">
                ...
                <div class="panel_s">
                    <div class="panel-body">
                     ...
                     <select name="item_select" class="selectpicker no-margin<?php if($ajaxItems == true){echo ' ajax-search';} ?><?php if(has_permission('items','','create')){ echo ' _select_input_group'; } ?>" data-width="100%" id="item_select" data-none-selected-text="<?php echo 'Inventory'; ?>" data-live-search="true">
                        <option value=""></option>
                        <?php foreach($items as $group_id=>$_items){ ?>
                        <optgroup data-group-id="<?php echo $group_id; ?>" label="<?php echo $_items[0]['group_name']; ?>">
                        <?php foreach($_items as $item){ 
						$iselect = '';
						if($item['id']==$expense->item_select) {
							$iselect = 'selected="selected"';
						}
						?>
                        <option <?php echo $iselect; ?> value="<?php echo $item['id']; ?>" >(<?php echo app_format_number($item['sku_code']); ; ?>) <?php echo $item['description']; ?></option>
                        <?php } ?>
                        </optgroup>
                        <?php } ?>
                    </select>
