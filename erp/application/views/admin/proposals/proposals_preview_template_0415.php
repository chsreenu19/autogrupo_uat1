        ...
        <div class="row mtop20">
            ...
            <div class="col-md-9 text-right _buttons proposal_buttons">
                ...
                <div class="btn-group ">
					...
					<?php 
			   if($proposal->status==3 || $proposal->status=='3'){
				
			   ?>
				<!--<li >
                     <a style="background: #3263d4; color: #fff;" href="<?php echo CUSTOMPDFFILESPATH; ?>dealclose.php?quoteid=<?php echo $proposal->id ?>" target="_blank">Deal Close</a>
                </li>-->
				
				<!--<li>
                     <a style="background: #e40f2b; color: #fff;" href="<?php echo admin_url(); ?>proposals/salesorder/<?php echo $proposal->id ?>" target="_blank">Sales Order</a>
                </li>-->
				
				<!--<li>
                     <a style="background: green; color: #fff;" href="javascript:void(0)" class="esignature" id="<?php echo $proposal->id; ?>">eSignature</a>
                </li>-->
				
				<!--<li>
                     <a target="blank" style="background: yellow; color: red;" href="<?php echo admin_url(); ?>creditapplication/client/<?php echo $proposal->id ?>" class="creditapplication" id="<?php echo $proposal->id; ?>">Credit Appliation</a>
                </li>-->
				
				<li>
                     <a style="background: grey; color: #fff;" href="javascript:void(0)" class="transferforms" id="<?php echo $proposal->id; ?>">Transfer Forms</a>
                </li>
				...
				<li>
                     <a target="blank" style="background: yellow; color: red;" href="<?php echo admin_url(); ?>creditapplication/client/<?php echo $proposal->id ?>" class="creditapplication" id="<?php echo $proposal->id; ?>">Credit Appliation</a>
						...
						<li>
                            ...
                            <a attr href="<?php echo admin_url() . 'proposals/mark_action_status/' . $status . '/' . $proposal->id; ?>"><?php echo _l('proposal_mark_as', format_proposal_status($status, '', false)); ?></a>
                            
                              
                               
                            <?php } else if($status==SOLD_INVOICE_PENDING_ID && $proposal->status==3){ 

                                 $formspermissionsroles = json_decode(QUOTEFORMS_ALLOWED_ROLES,TRUE);
                                 
                                 if(in_array($userroleid, $formspermissionsroles)){
                                    
                                 
                            ?>
                                
                                <a <?php echo $userroleid; ?> href="<?php echo admin_url() . 'proposals/mark_action_status/' . $status . '/' . $proposal->id; ?>"><?php echo _l('proposal_mark_as', format_proposal_status($status, '', false)); ?></a>
                ...
                <div class="btn-group">
                    ...
                    // } ?>><?php //echo _l('proposal_convert_invoice'); ?>
                     </a>-->
                ...
                <div class="tab-content">
                    <div role="tabpanel" class="tab-pane active" id="tab_proposal">
                     ...
                     if ($proposal->project_id != 0) { ?>
                        ...
                        if(IS_ESIGN_ENABLED && $proposal->status == 3 && ($userrole->role==FI_ROLE_ID || $userrole->role==SUPER_ADMIN_ROLE_ID)){
                        ?>
                            <p class="bold"><?php echo _l('eSignature Files'); ?></p>

                            <?php $esignforms = json_decode(ESIGN_FORM_NAMES,TRUE); ?> 
                            
                            <?php 
                            //echo "<pre>"; print_r($esignforms); exit;
                            foreach($esignforms as $row){
                            ?>
                            <div class="mbot15 row">
                                <div class="col-md-8">
                                
                                    <a href="<?php echo base_url() ?>admin/proposals/<?php echo $row['link'] ?>/<?php echo $proposal->id; ?>"
                                        target="_blank"><i class="fa fa-check-circle" aria-hidden="true"></i>&nbsp;&nbsp;&nbsp;<?php echo $row['name'] ?></a>
                                    <br />
                                </div>
                        ...
                        ?> 
						 ...
						 <!-- <div class="mbot15 row">
                            <div class="col-md-8">
                                ...
                                <a href="<?php echo base_url() ?>admin/proposals/salesorder/<?php //echo $proposal->id; ?>"
                                    target="_blank"><i class="fa fa-check-circle" aria-hidden="true"></i>&nbsp;&nbsp;&nbsp;<?php //echo 'Sales Order'; ?></a>
                                <br />
                            </div>
...
<form style="display:none;" id="esignForm" action=" https://esteps.firmadoc.co/api/v1/datos.php" method="post" target="_blank">
 <textarea name="token" id="token"></textarea>
 <textarea name="json_params" id="json_params"></textarea>
 <input type="submit">
</form>
...
$(document).ready(function(){
	$("body").on("click",".esignature",function(){
		var id = $(this).attr("id");
		$.ajax({
			type:'get',
			url: '<?php echo base_url(); ?>/admin/apiint/makerequesttoesignature/'+id,
			success:function(data){
				console.log(data);
				var json = $.parseJSON(data);	
				console.log(json.Data);
				//var jsond = '{ "trade_in": {"fecha": "2022-12-22","yo": "FRANCISCO JAVIER GIERBOLINI COLLAZO", "marca": "Hyundai", "modelo": "Venue", "tablilla": "NANANANA", "num_serie": "132132123", "num_cuenta": "456456456","banco": "Bank of America","balance_pendiente": "9999.00","num_poliza": "789789789", "contrato_servicio": "147852963"}';
				var jsond = JSON.stringify(jsond);
				if(typeof json.Token != 'undefined'){
					/*$('<form>', {
						"id": "getInvoiceImage",
						"html": '<textarea name="token" id="token">'+json.Token+'</textarea><textarea name="json_params" id="json_params">'+jsond+'</textarea><input type="submit">',
						"action": 'https://esteps.firmadoc.co/api/v1/datos.php',
						"target":"_blank"
					}).appendTo(document.body).submit();*/
					$("#token").val(json.Token);
					$("#json_params").val(JSON.stringify(json.Data));
					$("#esignForm").submit();
				}
			}
		});
	...
	$("body").on("click",".transferforms",function(){
		
		var urlList = "<?php echo base_url(); ?>/inv/rve_es/"+$(this).attr('id')+", <?php echo base_url(); ?>/inv/mvsl_es/"+$(this).attr('id')+" ";
		//alert(urlList);
