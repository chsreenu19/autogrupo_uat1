...
class Inv extends ClientsController
{
	...
	public function getdetailsbyquoteid($id){
		$curl = curl_init();

		curl_setopt_array($curl, array(
		  CURLOPT_URL => base_url().'admin/apiint/rve/'.$id,
	...
	public function rve($id){
		...
		$rveData = $this->getdetailsbyquoteid($id);
		//$rveData = json_decode($rveData,true);
		//echo "<pre>cc"; print_r($rveData); exit;
		if(count($rveData['main_data']['lead'])>0){
			$data['alldata']  = $rveData;
		}
