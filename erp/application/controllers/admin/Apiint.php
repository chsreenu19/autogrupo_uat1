...
class Apiint extends ApiController
{
	...
	public function getquoteandleaddetailsbyid($id){
		...
		if(count($check)>0){
			if(isset($check[0]['assigned']) && $check[0]['assigned']!=''){
				...
				//getstaff manager 
				$assignedstaffmanager = "SELECT team_manage FROM tblstaff WHERE staffid =".$check[0]['assigned'];
				$assignedstaffmanagercheck = $this->queryvaidator($assignedstaffmanager);
				if(count($assignedstaffmanagercheck)>0){
					$check[0]['assignedstaffmanager'] = get_staff_full_name($assignedstaffmanagercheck[0]['team_manage']);
				}else{
					$check[0]['assignedstaffmanager'] = '';
				}
			}
				...
				if(count($lcf)>0){
					...
					foreach($lcf as $lcfr){
						...
						///if(count($lcfrresult)>0){
							...
							$checkLeads[0][$lcfr['slug']]=isset($lcfrresult[0]['value']) ? $lcfrresult[0]['value'] : '';
			...
			if(count($ccheck)>0){
							...
							if(count($vinexplode)>0){
								...
								if(count($itemsresults)>0){
									...
									foreach($itemcustomfldsres as $r){
										...
										$pitemsresults[$i]['iattributes'][$r['slug']]=$r['value'];
										$pitemsresults[$i]['QPrice'] = $itm['rate'];
				...
				$tradeIndetails['millage'] = $carray['proposal_mileage'];
				...
				foreach($pitemsresults as $inv){
					if(isset($inv['iattributes'])){
						...
						$quoteInv['invid'] = $inv['id'];
						$quoteInv['invqprice'] = $inv['QPrice'];
					}
				}

				//echo "<pre>"; print_r($quoteInv); exit;
				...
				$warehouse = "SELECT warehouse_id, warehouse_code, warehouse_name,city FROM tblwarehouse";
				//echo $pmode; exit;
				$warehouseResults = $this->queryvaidator($warehouse);
				if(is_array($warehouseResults) && count($warehouseResults)>0){
					$result['warehouse']=$warehouseResults[0];
				}else{
	...
	function esignature($data){
		...
		// $array['factura_venta']['vehiculo_vendido_marbete'] = isset($data['quotecustomfields']['proposal_marbete']) ? $data['quotecustomfields']['proposal_marbete']:'';
		...
		$array['main_data']['warehouse'] = $data['warehouse'];			
	...
	public function updateinvidonitemabletable(){
		$qry = "SELECT * FROM tblitemable";
		...
		if(is_array($result) && count($result)>0){
			foreach($result as $row){
				$str = $row['description'];
				if($str!=''){
					if (strpos($str, '-vin-') !== false) {
						$explodeStr = explode(" ",$str);
					   
						if(is_array($explodeStr) && count($explodeStr)>0){
							$vin = trim(end($explodeStr));
							$pitems = "select id,sku_code from tblitems where sku_code= '".$vin."'";
							$pitemsresults = $this->db->query($pitems);
							$presults = $pitemsresults->result_array();   
							//echo "<pre>"; print_r($presults); exit;
							if(is_array($presults) && count($presults)>0){
								$invId = $presults[0]['id'];
								$vincode = $vin;
								$data = array(
										'inv_id' => $invId,
								...
								$this->db->update('tblitemable', $data);
							}
						}
					}
				}
