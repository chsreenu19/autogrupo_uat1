...
class Creditapplication extends AdminController
{
    /* List all clients */
    public function index()
    {
        ...
        $this->load->view('admin/creditapplication/manage', $data);
    }
    ...
    public function client($id)
    {
		...
		//echo base_url().'admin/apiint/getquoteandleaddetailsbyid/'.$id; exit;
			
		$curl = curl_init();

		curl_setopt_array($curl, array(
		  CURLOPT_URL => base_url().'admin/apiint/creditapplicationdatabyquoteid/'.$id,
		...
		$chdata = curl_exec($curl);	
		$decodeData = json_decode($chdata,true); 
		
		
		
		
		
		//echo "<pre>dddd"; print_r($decodeData); exit;
		
		
		//echo count($decodeData['main_data']['lead']); exit;
		if(isset($decodeData['main_data']['lead']) && count($decodeData['main_data']['lead'])>0){
			$data['title']     = $decodeData['main_data']['lead']['name'].' - Credit Applicaction';
			$data['alldata'] = $decodeData;
		}
		//echo "<pre>"; print_r($decodeData); exit;
				
			
        $data['bodyclass'] = 'customer-profile dynamic-create-groups';
        
		//echo "<pre>"; print_r($data); exit;
        $this->load->view('admin/creditapplication/client', $data);
    }
