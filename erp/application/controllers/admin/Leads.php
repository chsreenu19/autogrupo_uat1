...
class Leads extends AdminController
{
    ...
    private function _get_lead_data($id = '')
    {
        ...
        if (is_numeric($id)) {
            ...
            $lcf1 = $this->db->query($leadCustomFields);
            $lcf = $lcf1->result_array();
            //echo "<pre>"; print_r($lcf); exit;
            $checkLeads = array();
            if(count($lcf)>0){
                $carray =array();
                $k=0;
                ...
                foreach($lcf as $lcfr){
                    ...
                    //echo $lcfrvalues; exit;
                    $lcfrresult1 = $this->db->query($lcfrvalues);
                    $lcfrresult = $lcfrresult1->result_array();
                   // echo "<pre>"; print_r($lcfrresult); exit;
                    ///if(count($lcfrresult)>0){
                        $checkLeads[$lcfr['name']]=isset($lcfrresult[0]['value']) ? $lcfrresult[0]['value'] : '';					
                    //}
                    //echo "<pre>"; print_r($ccvaluesresult); exit;
                    
                }
            }
            $data['lead_cfields'] = $checkLeads;

            //echo "<pre>w"; print_r( $data['lead_cfields']); exit;
            ...
            if (is_gdpr() && get_option('gdpr_enable_consent_for_leads') == '1') {
                ...
                if(count($data['consents'])>0){
                    $ssnconsent = $this->gdpr_model->ssnconsent($lead->id);
                   if(count($ssnconsent)>0){
                     $ssn_consent = $ssnconsent[0]['action'];
                     
                     $data['ssnconsent'] = $ssn_consent;
                   }else{
                    $data['ssnconsent'] = '';
                   }
                }
    ...
    public function ableittcredditcheckapi(){
        $data = $_REQUEST;
		//echo "<pre>"; print_r($data); exit;
		$curl = curl_init();

		curl_setopt_array($curl, array(
		  CURLOPT_URL => 'https://www.700creditsolution.com/XCRS/Service.aspx',
		  ...
		  CURLOPT_POSTFIELDS => "ACCOUNT=auto1pq&
									PASSWD=AUT@700credit&
									pass=2&
									process=PCCREDIT&
									NAME=".$data['cname']."&
									...
									product=PREQUALIFY&
									email=".$data['cemail']."&
									BUREAU=TU&APPEXPIN=123456&
									TVINNUMBER=WP0ZZZ99ZTS392124",
	...
	public function creditreport($id=''){
		//echo CREDIT_CHECK_CUSTOM_FIELD_ID; EXIT;
		//echo $id; exit;
		if(isset($id) && $id!=''){
			$data['title'] = 'Credit Report';
			$qry = "SELECT 
						tblcustomfieldsvalues.value AS HTMLLink, 
						tblleads.name AS LeadName 
					FROM tblcustomfieldsvalues 
					LEFT JOIN tblleads ON tblleads.id=tblcustomfieldsvalues.relid 
					WHERE relid=$id AND fieldid=".CREDIT_CHECK_CUSTOM_FIELD_ID;
			...
			if(is_array($result) && count($result)>0){
				...
				$data['title'] =  $result[0]['LeadName'].' - Credit Report';
				$data['htmllink'] = $result[0]['HTMLLink'];
				$this->load->view('admin/leads/creditreport', $data);
			}else{
				$data['title'] = 'Credit Report';
				exit('No credit record found.');
			}
		
			
		}else{
			$data['title'] = 'Credit Report';
			exit('Unauthourized Access');
		}
    ...
    public function ableittcredditcheckapibyleadid($leadId){

        $leadetails = $this->getleaddetailsbyleadid($leadId);
    
       // echo "<pre>ddd"; print_r($leadetails['leadData']); exit;
        if(count($leadetails)>0){

            

            $data = $leadetails['leadData'];

            if($data['Social Security Number']==''){
                echo 'SSN is empty/not valid';
            }else if($data['name']==''){
                echo 'Lead name is empty/not valid';
            }
            else if($data['address']==''){
                echo 'Address is empty/not valid';
            }
            else if($data['city']==''){
                echo 'City is empty/not valid';
            }
            else if($data['state']==''){
                echo 'State is empty/not valid';
            }
            else if($data['zip']==''){
                echo 'Zip is empty/not valid';
            }
            else if($data['email']==''){
                echo 'Email is empty/not valid';
            }else{
            //
                $curl = curl_init();
                ...
                if(isset($array['XML_Report'])){
                    if(isset($array['XML_Report']['Prescreen_Report']['ResultCode'])){
                        
                         if($array['XML_Report']['Prescreen_Report']['ResultCode']=='A'){
                        
                            $creditScore = $array['XML_Report']['Prescreen_Report']['Score'];
                            $custom_report = $array['custom_report'];

                            $inArray = array(CUSTOM_FIELD_CREDIT_SCORE=>$creditScore, CUSTOM_FIELD_700_CUSTOM_REPORT=>$custom_report);
                            
                            foreach($inArray as $key=>$r){
                
                                $ifexists = "SELECT * FROM tblcustomfieldsvalues WHERE fieldid=$key AND relid=$leadId ";
                                $ifexistsexe = $this->db->query($ifexists);
                                $ifexistresult = $ifexistsexe->result_array();
                                if(count($ifexistresult)>0){
                                    $updateqry = "UPDATE tblcustomfieldsvalues SET value='".$r."' WHERE id=". $ifexistresult[0]['id'];
                                    $this->db->query($updateqry);
                                }else{
                                    $insQry = "INSERT INTO tblcustomfieldsvalues (relid, fieldid, fieldto, value) values($leadId,$key,'leads','".$r."')";
                                    $this->db->query($insQry);
                                }
                            }
                            echo "Credit Score/ Report fetched successfully";
                            //echo "<pre>"; print_r($inArray);exit;
                            //echo $creditScore.'=>'.$custom_report; exit;
                        }else{
    ...
    public function getleaddetailsbyleadid($leadId){
        $leaddata = "SELECT * FROM tblleads WHERE id =".$leadId;
			$checkLds = $this->db->query($leaddata);
            $checkLeads = $checkLds->result_array();
			...
			if(count($checkLeads )>0){
				...
				if(count($lcf)>0){
					...
					foreach($lcf as $lcfr){
						$lcfrvalues = $cquery = "select * from tblcustomfieldsvalues where fieldid ='".$lcfr['id']."' and relid= $leadId";
						$lcfrres = $this->db->query($lcfrvalues);
                        $lcfrresult = $lcfrres->result_array();
						//echo "<pre>"; print_r($ccvaluesresult); exit;
						///if(count($lcfrresult)>0){
