...
class Invoice_items extends AdminController
{
    ...
    public function get_item_by_id($id)
    {
        if ($this->input->is_ajax_request()) {
			...
			//ableit
			$dyn = $this->invoice_items_model->getcustomfieldsbyid($id);
			/*if(count($dyn)>0){
				$cFields = array();
				$i=0;
				foreach($dyn as $rr){
					//echo "<pre>"; print_r($rr); exit;
					$cFields[$rr['clm']]= $rr['value'];
					$i++;
				}
				unset($cFields['Exteriors']);
				unset($cFields['Interiors']);
				unset($cFields['Security']);
				unset($cFields['Mechanical']);
				unset($cFields['Entertainment']);
				$item->dynamic = $cFields;
			}*/
			...
			if(count($dyn)>0){
				...
				foreach($dyn as $rr){
					//echo "<pre>"; print_r($rr); exit;
					if($rr['clm']=='Marbete Vence'){
						if($rr['value']!=''){
							$cFields[$rr['clm']]= date("m/d/Y",strtotime($rr['value']));
						}
					}else{
					$cFields[$rr['clm']]= $rr['value'];
					}
