...
class Estimates_model extends App_Model
{
    ...
    public function convert_to_invoice($id, $client = false, $draft_invoice = false)
    {
        ...
        if ($id) {
			...
			//ableittech 04092024 Update proposal with the invoice_id
            $this->db->where('estimate_id', $_estimate->id);
	...
	public function getproposalid($estimate_id){

        $pid = "SELECT * FROM ".db_prefix()."proposals WHERE estimate_id=".$estimate_id;
        $pidExe = $this->db->query($pid);
        $pidResult = $pidExe->result_array();
        //echo "<pre>"; print_r($pidResult); exit;
        if(is_array($pidResult) && count($pidResult)>0){
         $acvids = implode(",", json_decode(IDS_FOR_CALCULATION_ACV,true));
         $qry = "SELECT * FROM ".db_prefix()."customfieldsvalues WHERE fieldid IN(".$acvids.") AND  fieldto ='proposal' AND relid = (SELECT id FROM tblproposals WHERE estimate_id=".$estimate_id.")";
         //echo $qry; exit;
         ...
         if(is_array($result) && count($result)>0){
             ...
             foreach($result as $key=>$row){
                 ...
                 //echo $row['id'].'<br/>'; 
                 $keyvalue[$row['fieldid']] = $row['value'];
             }
             $retArray = array('id'=>$pidResult,'acvdata'=>$keyvalue);
             return $retArray;
         }else{
             return array();
         }
     ...
     public function getjournalentriesbyestimateid($estimate_id){
        $query = "SELECT * FROM ".db_prefix()."ableit_jnl_automated  WHERE estimate_id=".$estimate_id;
        $exe = $this->db->query($query);
        return $exe->result_array();
     }

     public function getjournalentriesbyestimateidforinvoice($estimate_id){
        $query = "SELECT * FROM ".db_prefix()."ableit_jnl_automated  WHERE estimate_id=".$estimate_id." AND is_jnl_posted=0";
        $exe = $this->db->query($query);
        return $exe->result_array();
     }

     public function getcommodityidbyvin($vin){
        $query = "SELECT * FROM ".db_prefix()."items  WHERE sku_code='".$vin."' ";
        $exe = $this->db->query($query);
        return $exe->result_array();
     }
	 
	 public function dcjournalentries($estimate_id){
        $qry ='SELECT 
                    a.control_type,
                    a.attr_control_number,
                    a.attr_sales_rep_id,
                    CONCAT(s.firstname," ",s.lastname) AS sales_rep_name,
                    a.attr_vin,
                    a.attr_cust_lead_id,
                    l.name AS lead_name,
                    a.attr_description,
                    ...
                    a.attr_debit_coa,
                    a.attr_credit_coa,
                    CONCAT("(",ac.number,") ",ac.name) AS credit_coa_name,
                    CONCAT("(",ad.number,") ",ad.name) AS debit_coa_name,
                    a.estimate_id,
                    a.quote_id,
                    a.lead_id,
                    a.is_jnl_posted,
                    ...
                    a.invoice_id
                FROM 
                    tblableit_jnl_automated a
                JOIN 
                    tblacc_accounts ac ON a.attr_credit_coa = ac.id
                JOIN 
                    tblacc_accounts ad ON a.attr_debit_coa = ad.id    
                JOIN 
                    tblstaff s ON a.attr_sales_rep_id = s.staffid
                JOIN 
                    tblleads l ON a.attr_cust_lead_id = l.id
                WHERE 
                    a.estimate_id = '.$estimate_id;
        $qryexe = $this->db->query($qry)->result_array();
        return $qryexe;      
     }

     public function dcproposalcustomfields($proposalid){
       
        $cquery = "select * from tblcustomfields where fieldto ='proposal' and active=1";			
        $ccheck = $this->db->query($cquery)->result_array();  
        $carray =array();      
        if(count($ccheck)>0){           
            $k=0;
            foreach($ccheck as $ck){
                $ccvalues = $cquery = "select * from tblcustomfieldsvalues where fieldid ='".$ck['id']."' and relid= $proposalid";
                $ccvaluesresult =$this->db->query($ccvalues)->result_array();
     ...
     public function dcexpenses($itemid){
        $qry = $this->db->query("SELECT 
                                    ...
                                    a.amount
                                FROM tblexpenses a
                                LEFT JOIN tblexpenses_categories b ON b.id=a.category
                                WHERE a.item_select=".$itemid);
        return $qry->result_array();
     }

     public function dcavcentries($estimate_id){
        $qry = $this->db->query("
                                SELECT *
                                FROM
                                ".db_prefix()."acc_journal_entries
                                where  description like '%ACV-Entry%' AND abltit_estimate_id=".$estimate_id);
        ...
        if(count($result)>0){
            ...
            foreach($result as $row){
                ...
                $getach = $this->db->query(
									"SELECT 
										ah.rel_id AS 'ahid', 
										ah.creditgl, 
										ah.debitgl, 
										cr.name AS creditcoa, 
										dr.name AS debitcoa 
									FROM (
										SELECT 
											rel_id, 
											MAX(CASE WHEN rn = 1 THEN `account` END) AS creditgl, 
											MAX(CASE WHEN rn = 2 THEN `account` END) AS debitgl
										FROM (
											SELECT 
												rel_id, 
												`account`,
												@rn := IF(@prev_rel_id = rel_id, @rn + 1, 1) AS rn,
												@prev_rel_id := rel_id
											FROM 
												tblacc_account_history 
											CROSS JOIN (SELECT @rn := 0, @prev_rel_id := NULL) AS vars
											WHERE 
												rel_id = ".$row['id']." and rel_type='journal_entry'
											ORDER BY 
												rel_id
										) AS numbered
										GROUP BY 
									...
									LEFT JOIN tblacc_accounts AS cr ON cr.id = ah.creditgl
									LEFT JOIN tblacc_accounts AS dr ON dr.id = ah.debitgl;");
									

                $getachresult = $getach->result_array();
                if(count($getachresult)>0){
                    foreach($getachresult[0] as $key=>$ach){
                        $result[$i][$key] = $ach;
                    }
                }
                $i++;
