...
class Proposals_model extends App_Model
{
    ...
    public function getproposalcount(){
        $qry = "SELECT count(id) as 'QuotesCount' from ".db_prefix() . "proposals WHERE YEAR(datecreated) = YEAR(NOW())";
        $exe = $this->db->query($qry);
        $result = $exe->result_array();
        if(is_array($result) && count($result)>0){
            return number_format($result[0]['QuotesCount']);
        }else{
            return 0;
        }
    ...
    public function getfrontsalestotal(){
        $qry = "SELECT sum(total) as 'QuotesSum' from ".db_prefix() . "proposals WHERE YEAR(datecreated) = YEAR(NOW())";
        $exe = $this->db->query($qry);
        $result = $exe->result_array();
        if(is_array($result) && count($result)>0){
            return '$'.number_format($result[0]['QuotesSum']);
        }else{
            return  '$0';
        }
    ...
    public function getbacksalestotal(){
        $ids = implode(",",json_decode(BACKSALES_IDS,true));
        $qry = "SELECT SUM(b.value) AS 'backsales'
        FROM tblcustomfieldsvalues b
        LEFT JOIN tblproposals a ON b.relid = a.id 
        WHERE b.fieldid IN ($ids) AND b.fieldto='proposal' AND YEAR(a.datecreated) = YEAR(NOW())";

        $exe = $this->db->query($qry);
        $result = $exe->result_array();
       // echo "<pre>"; print_r($result); exit;
        if(count($result)>0){
            return '$'.number_format($result[0]['backsales']);
        }else{
            return  '$0';
        }
    ...
    public function getinvoicedtotal(){
        $qry = "SELECT SUM(total) AS 'InvoicedTotal' FROM tblproposals WHERE invoice_id IS NOT NULL AND YEAR(datecreated) = YEAR(NOW())
        ";
        $exe = $this->db->query($qry);
        $result = $exe->result_array();
        if(is_array($result) && count($result)>0){
            return '$'.number_format($result[0]['InvoicedTotal']);
        }else{
            return '$0';
        }
	...
	public function getinsurancetotal(){
        $ids = implode(",",json_decode(INSURANCE_IDS,true));
        $qry = "SELECT SUM(b.value) AS 'insurancesales'
        FROM tblcustomfieldsvalues b
        LEFT JOIN tblproposals a ON b.relid = a.id 
        WHERE b.fieldid IN ($ids) AND b.fieldto='proposal' AND YEAR(a.datecreated) = YEAR(NOW())";
       ...
       //echo "<pre>"; print_r($result); exit;
        if(count($result)>0){
            return '$'.number_format($result[0]['insurancesales']);
        }else{
            return  '$0';
        }
