...
class Invoice_items_model extends App_Model
{
	...
	//ableit
	public function getcustomfieldsbyid($id){
			$query = $this->db->query("SELECT a.relid, b.name AS 'clm',a.value FROM `tblcustomfieldsvalues` a 
						LEFT JOIN `tblcustomfields` b ON a.fieldid=b.id WHERE a.fieldto = 'items_pr' AND a.relid=$id");
	...
	public function get_grouped_expenses_screen()
    {
        $items = [];
        $this->db->order_by('name', 'asc');
        $groups = $this->db->get(db_prefix() . 'items_groups')->result_array();
        ...
        foreach ($groups as $group) {
            ...
            if (count($_items) > 0) {
                ...
                /*foreach ($_items as $i) {					
					$relid = $i['id'];
					$ccvalues = $this->db->query("select * from tblcustomfieldsvalues where fieldid =".CUSTOM_FIELD_ITEM_AVAILABLE_STATUS." and relid= $relid");
                    //echo "select * from tblcustomfieldsvalues where fieldid =".CUSTOM_FIELD_ITEM_AVAILABLE_STATUS." and relid= $relid"; exit;
					$ccvaluesResults = $ccvalues->result_array();
                   
					if(is_array($ccvaluesResults) && count($ccvaluesResults)>0){
						if($ccvaluesResults[0]['value']=='For Sale'){
							array_push($items[$group['id']], $i);
						}
					}else{
						array_push($items[$group['id']], $i);
	...
	public function get_grouped_quote()
    {
        $items = [];
        $this->db->order_by('name', 'asc');
        $this->db->where('id',1);
        ...
        foreach ($groups as $group) {
            ...
            if (count($_items) > 0) {
                ...
                foreach ($_items as $i) {
                    //array_push($items[$group['id']], $i);
					$check_if_inventory_is_added_to_quote = check_if_inventory_is_added_to_quote($i['id']);
    ...
    public function search($q)
    {
		...
		$this->db->where("(description LIKE '%".$q."%' OR long_description LIKE '%".$q."%' OR commodity_code LIKE '%".$q."%')", NULL, FALSE);
