...
/**
    ...
    */
    ...
    public function abl_it_add_pur_order($data){ 
        ...
        $check_appr = $this->get_approve_setting('pur_order');
        // echo "<Pre>";print_r($check_appr);exit;
        $data['approve_status'] = 1;
        if($check_appr && $check_appr != false){
            $data['approve_status'] = 1;
        }else{
        ...
        if ($insert_id) {
            ...
            if(count($order_detail) > 0){
                foreach($order_detail as $key => $rqd){ 
                    // echo $key.'...'.$rqd['quantity'];exit;
                    $dt_data = [];
                    $dt_data['pur_order'] = $insert_id;
                    // $dt_data['item_code'] = $rqd['sku_code'];
                    $dt_data['item_code'] = $rqd['id'];
                    $dt_data['unit_id'] = isset($rqd['unit_id']) ? $rqd['unit_id'] : null;
                    $dt_data['unit_price'] = $rqd['purchase_price'];
                    //$dt_data['into_money'] = $rqd['into_money'];
                    $dt_data['total'] = $rqd['purchase_price'];
                    $dt_data['tax_value'] = 0.00;
                    $dt_data['item_name'] = $rqd['commodity_code'];
                    $dt_data['description'] = $rqd['long_description'];
                    $dt_data['total_money'] = $rqd['purchase_price'];
                    ...
                    // $dt_data['quantity'] = ($rqd['quantity'] != ''&& $rqd['quantity'] != null) ? $rqd['quantity'] : 0;
                    $dt_data['quantity'] = (isset($rqd['quantity']) && $rqd['quantity'] != ''&& $rqd['quantity'] != null) ? $rqd['quantity'] : 1;
                    // echo "<pre>sss";print_r($dt_data);exit;
                    $this->db->insert(db_prefix().'pur_order_detail', $dt_data);
