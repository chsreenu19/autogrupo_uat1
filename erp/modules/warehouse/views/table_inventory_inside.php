	...
	foreach ($rResult as $aRow) {
		...
		for ($i = 0; $i < count($aColumns); $i++) {
			...
			elseif($aColumns[$i] == 'commodity_barcode') {
				...
				$expQry = 'SELECT SUM(amount) as expenses from '.db_prefix() .'expenses where item_select='.$aRow['id'];
				//echo $expQry; //exit;
				
				$expExe = $this->ci->db->query($expQry);
				$expResult = $expExe->result_array();
				if(is_array($expResult) && count($expResult)>0){
					if($expResult[0]['expenses']>0){
					$_data =	'<a target="_blank" href="' . admin_url('warehouse/view_commodity_detail/' . $aRow['id']) . '#veh_expenses" >'. app_format_money((float)$expResult[0]['expenses'],'') .'</a>';
					}else{
						$_data = '0.00';
					}	
