...
if($psearchStartDate!='' && $psearchToDate==''){
   // echo 'here'; exit;
    array_push($where, 'AND DATE(created_on)= "'.$psearchStartDate.'" ');
    //$where[] = ' AND DATE(datecreated)= ' . $psearchStartDate;
}

if($psearchStartDate!='' && $psearchToDate!=''){
   
    array_push($where, 'AND (DATE(created_on) BETWEEN "'.$psearchStartDate.'" AND  "'.$psearchToDate.'")');
...
if($itm_available_status_filter =='For Sale' || $itm_available_status_filter == 'Sold' && $itm_available_status_filter!='0'){
	...
	array_push($where, 'AND '.db_prefix().'customfieldsvalues.fieldid IN ('.INV_AVAILABLE_STATUS_FIELD_ID.') AND '.db_prefix().'customfieldsvalues.fieldto="items_pr" and '.db_prefix().'customfieldsvalues.value="For Sale"');
}
	...
	foreach ($rResult as $aRow) {
		...
		for ($i = 0; $i < count($aColumns); $i++) {
			...
			if ($aColumns[$i] == 'commodity_code') {
				...
				$_data = app_format_money($aRow['rate'], ($aRow['currency'] != 0 ? get_currency($aRow['rate']) : $baseCurrency));
			} elseif ($aColumns[$i] == 'purchase_price') {
				$_data = app_format_money($aRow['purchase_price'], ($aRow['currency'] != 0 ? get_currency($aRow['purchase_price']) : $baseCurrency));

			} elseif ($aColumns[$i] == 'commission_pack') {
				$_data = app_format_money($aRow['commission_pack'], ($aRow['currency'] != 0 ? get_currency($aRow['commission_pack']) : $baseCurrency));
