...
if($itm_available_status_filter == 'Consignment' && $itm_available_status_filter!='0'){
	...
	array_push($where, 'AND '.db_prefix().'customfieldsvalues.fieldid IN ('.INV_AVAILABLE_STATUS_FIELD_ID.') AND '.db_prefix().'customfieldsvalues.fieldto="items_pr" and '.db_prefix().'customfieldsvalues.value="Consignment"');
}
	...
	foreach ($rResult as $aRow) {
		...
		for ($i = 0; $i < count($aColumns); $i++) {
			...
			if ($aColumns[$i] == 'commodity_code') {
				...
				$_data = app_format_money($aRow['rate'], (isset($aRow['currency']) && $aRow['currency'] != 0 ? '$'.get_currency($aRow['rate']) : $baseCurrency));
