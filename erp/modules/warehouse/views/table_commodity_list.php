...
$aColumns = [
	...
	'sale_gl_account',
	'purchase_gl_account',
	//'DATEDIFF(CURDATE(),STR_TO_DATE(purchase_date, "%m/%d/%Y")) as aging',
	'DATEDIFF(NOW(), purchase_date) as aging',
	'commission_pack',
	'tax',
	'origin',
	'2',	//minimum stock
...
if (isset($groups_filter)) {
	...
	$where[] = 'AND '.db_prefix().'items.group_id IN (' . $groups_filter . ')';
...
if($itm_available_status_filter =='For Sale' || $itm_available_status_filter == 'Sold' && $itm_available_status_filter!='0'){
	...
	array_push($where, 'AND '.db_prefix().'customfieldsvalues.fieldid IN ('.INV_AVAILABLE_STATUS_FIELD_ID.') AND '.db_prefix().'customfieldsvalues.fieldto="items_pr" and '.db_prefix().'customfieldsvalues.value="'.$itm_available_status_filter.'"');
}
...
$rResult = $result['rResult'];
	...
	foreach ($rResult as $aRow) {
		$row = [];
		$check_if_inventory_is_added_to_quote = check_if_inventory_is_added_to_quote($aRow['id']);
		//echo "<pre>";print_r($check_if_inventory_is_added_to_quote); exit;
		...
		for ($i = 0; $i < count($aColumns); $i++) {
			...
			if ($aColumns[$i] == 'commodity_code') {
				...
				if (has_permission('warehouse', '', 'edit') || is_admin()) {
					if(empty($check_if_inventory_is_added_to_quote)){
					$code .= ' | <a href="#" onclick="edit_commodity_item(this); return false;"  data-commodity_id="' . $aRow['id'] . '" data-description="' .  $aRow['description'] . '" data-long_description="' . $aRow['long_description'] . '" data-unit_id="' . $aRow['unit_id'] . '" data-commodity_code="' . $aRow['commodity_code'] . '" data-commodity_barcode="' . $aRow['commodity_barcode'] . '" data-commodity_type="' . $aRow['commodity_type'] . '" data-origin="' . $aRow['origin'] . '" data-color_id="' . $aRow['color_id'] . '" data-style_id="' . $aRow['style_id'] . '" data-model_id="' . $aRow['model_id'] . '" data-size_id="' . $aRow['size_id'] . '"  data-rate="' . app_format_money($aRow['rate'], '') . '" data-group_id="' . $aRow['group_id'] . '" data-tax="' . $aRow['tax'] . '"  data-warehouse_id="' . $aRow['warehouse_id'] . '" data-sku_code="' . $aRow['sku_code'] . '" data-sku_name="' . $aRow['sku_name'] . '" data-sub_group="' . $aRow['sub_group'] . '" data-purchase_price="' . $aRow['purchase_price'] . '" data-purchase_date="' . $aRow['purchase_date'] . '" data-purchase_gl_account="' . $aRow['purchase_gl_account']. '" data-sale_gl_account="' . $aRow['sale_gl_account'] .'" data-commission_pack="' . $aRow['commission_pack'] . '" data-color="' . $aRow['color'] . '" data-guarantee="' . $aRow['guarantee'] . '" data-profif_ratio="' . $aRow['profif_ratio'] . '" data-without_checking_warehouse="' . $aRow['without_checking_warehouse'] . '" data-parent_id="' . $aRow['parent_id'] . '" >' . _l('edit') . '</a>';
					}elseif(count($check_if_inventory_is_added_to_quote)>0 && $check_if_inventory_is_added_to_quote['proposal_status']!=7){
						$code .= ' | <a href="#" onclick="edit_commodity_item(this); return false;"  data-commodity_id="' . $aRow['id'] . '" data-description="' .  $aRow['description'] . '" data-long_description="' . $aRow['long_description'] . '" data-unit_id="' . $aRow['unit_id'] . '" data-commodity_code="' . $aRow['commodity_code'] . '" data-commodity_barcode="' . $aRow['commodity_barcode'] . '" data-commodity_type="' . $aRow['commodity_type'] . '" data-origin="' . $aRow['origin'] . '" data-color_id="' . $aRow['color_id'] . '" data-style_id="' . $aRow['style_id'] . '" data-model_id="' . $aRow['model_id'] . '" data-size_id="' . $aRow['size_id'] . '"  data-rate="' . app_format_money($aRow['rate'], '') . '" data-group_id="' . $aRow['group_id'] . '" data-tax="' . $aRow['tax'] . '"  data-warehouse_id="' . $aRow['warehouse_id'] . '" data-sku_code="' . $aRow['sku_code'] . '" data-sku_name="' . $aRow['sku_name'] . '" data-sub_group="' . $aRow['sub_group'] . '" data-purchase_price="' . $aRow['purchase_price'] . '" data-purchase_date="' . $aRow['purchase_date'] . '" data-purchase_gl_account="' . $aRow['purchase_gl_account']. '" data-sale_gl_account="' . $aRow['sale_gl_account'] .'" data-commission_pack="' . $aRow['commission_pack'] . '" data-color="' . $aRow['color'] . '" data-guarantee="' . $aRow['guarantee'] . '" data-profif_ratio="' . $aRow['profif_ratio'] . '" data-without_checking_warehouse="' . $aRow['without_checking_warehouse'] . '" data-parent_id="' . $aRow['parent_id'] . '" >' . _l('edit') . '</a>';
					}
					
				}
				
				if(is_array($check_if_inventory_is_added_to_quote) && count($check_if_inventory_is_added_to_quote)>0){

					
						$code .= ' | <a  target="blank" href="' . admin_url('proposals/list_proposals/' . $check_if_inventory_is_added_to_quote['rel_id']) . '" class="text-warning">' . _l('Quote') . '</a>';
				...
				$_data = _d($aRow['purchase_date']);
				
				
				

			}elseif ($aColumns[$i] == 'aging') {
				$_data = $aRow['aging'];
			...
			elseif ($aColumns[$i] == 'commission_pack') {
				$_data = app_format_money((float) $aRow['commission_pack'], '');
