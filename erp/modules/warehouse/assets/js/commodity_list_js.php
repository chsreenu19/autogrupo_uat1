...
warehouse_type_value = warehouse_type;
  ...
  var expenseDropzone;
   ...
   function expenseSubmitHandler(form){
	...
	data.purchase_date = $('input[name="purchase_date"]').val();
	data.commission_pack = $('input[name="commission_pack"]').val();
	...
	//ableit adding inv gl accounts
    data.purchase_gl_account = $('select[name="purchase_gl_account"]').val();
    data.sale_gl_account = $('select[name="sale_gl_account"]').val();
         ...
         if (response.commodityid) {
           if(typeof(expenseDropzone) !== 'undefined'){
            if (expenseDropzone.getQueuedFiles().length > 0) {
              ...
              expenseDropzone.on('complete', function() {
                
                //sycnitemstostore(response.commodityid);
              });
			 // sycnitemstostore(response.commodityid);	
              expenseDropzone.processQueue();
              //sycnitemstostore(response.commodityid);	
    ...
    function sycnitemstostore(id){
      $.ajax({
                  url: "<?php echo base_url(); ?>admin/warehouse/syncitems/"+id,
                  type: 'GET',
    ...
    function edit_commodity_item(invoker){
	  ...
	  tinymce.get('long_description').setContent($(invoker).data('long_description'));
	  ...
	  $('#commodity_list-add-edit input[name="purchase_date"]').val($(invoker).data('purchase_date'));
	  $('#commodity_list-add-edit input[name="commission_pack"]').val($(invoker).data('commission_pack'));

	//ivn gl accounts
    if($(invoker).data('purchase_gl_account') != ''){
      $('#commodity_list-add-edit select[name="purchase_gl_account"]').val($(invoker).data('purchase_gl_account')).change();
    }else{
      $('#commodity_list-add-edit select[name="purchase_gl_account"]').val('').change();
    }


    if($(invoker).data('sale_gl_account') != ''){
      $('#commodity_list-add-edit select[name="sale_gl_account"]').val($(invoker).data('sale_gl_account')).change();
    }else{
      $('#commodity_list-add-edit select[name="sale_gl_account"]').val('').change();
    }
    ...
    $.post(admin_url + 'warehouse/get_item_longdescriptions/'+$(invoker).data('commodity_id')).done(function(response) {
	  ...
	  var vinexists = $('input[name="custom_fields[items][1]"]').val();
      
      if(vinexists!=''){       
        $('input[name="custom_fields[items][1]"]').attr("readonly",true);
        //$('input[name="custom_fields[items][1]"]').attr('readonly',true);
      }
		//tinyMCE.activeEditor.setContent("response.custom_fields_html");
		  tinyMCE.remove();
		  tinyMCE.init({
			selector: 'textarea',
			...
			$('#custom_fields_items').find('textarea').length
            //get tags
