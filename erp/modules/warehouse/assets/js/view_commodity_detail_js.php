...
var ProposalServerParams = {
  ...
  var   table_expenses_history = $('table.table-table_expenses_history');
   var  _table_api = initDataTable(table_expenses_history, admin_url+'warehouse/table_expenses_history', [], [], ProposalServerParams, [0, 'desc']);

   $('.table-table_expenses_history').DataTable().columns([0]).visible(false, false);

    $.each(ProposalServerParams, function(i, obj) {
        $('select' + obj).on('change', function() {  
            table_expenses_history.DataTable().ajax.reload()
                .columns.adjust()
                .responsive.recalc();
        });
    });
...
/*$("body").on("change",".expckb",function(){
	var value = 0;
	var id = $(this).attr("id");
	if($(this).is(':checked')){
		value = 1;
	...
	$.ajax({
		type: 'post',
		url: admin_url+'warehouse/updateitemselect',
...
function updateitemselect(id, expensesTotal, purchasePrice){
	$("body").on("change",".expckb",function(){
	var value = 0;
	var upValue;
	var id = $(this).attr("id");
	if($(this).is(':checked')){
		value = 1;
		upValue = purchasePrice + expensesTotal;
		
	}else{
		upValue = purchasePrice;
	}
	obj2 = new Intl.NumberFormat('en-US');  
	 output2 = obj2.format(upValue);  
	$.ajax({
		type: 'post',
		url: admin_url+'warehouse/updateitemselect',
		data:{"value": value, "id":id},
		success:function(data){
			$("span#purprice").text(output2);
		}
	})
});
}
