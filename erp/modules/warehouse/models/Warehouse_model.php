...
class Warehouse_model extends App_Model {
	 ...
	 * @return array or object
	 */
	public function get_commodity_group_type_all_ableit($id = false) {
		...
		if ($id == false) {
			$itemgroupIds = implode(",",json_decode(VEHICLE_GROUP_ITEM_SCREEN_IDS,true));
			return $this->db->query('select * from tblitems_groups where id in ('.$itemgroupIds.')')->result_array();
		}
	...
	public function get_warehourse_attachments_ableit($commodity_id) {
		$query = $this->db->query("SELECT * FROM `tblfiles` where `rel_id` = $commodity_id AND `rel_type` = 'commodity_item_file' ORDER BY `dateadded` DESC");
	...
	public function add_commodity_one_item($data) {
		...
		// echo "lll<Pre>";print_r( $arr_insert_cf);exit;
...
public function delete_goods_receipt($id) {
	...
	public function getcustomfieldsvaluebyinventoryid_ableit($id){
		$qry = "SELECT tblcustomfieldsvalues.value, tblcustomfields.type,tblcustomfields.name
				FROM tblcustomfieldsvalues JOIN tblcustomfields ON tblcustomfields.id=tblcustomfieldsvalues.fieldid 
				WHERE tblcustomfieldsvalues.relid = $id
				AND tblcustomfieldsvalues.fieldto = 'items_pr'";
		$exe = $this->db->query($qry);
		return $exe->result_array();
	}
	
	public function getitemavailablestatus($id){
		$qry = "SELECT * FROM ".db_prefix()."customfields WHERE id = ".$id;
		$exe = $this->db->query($qry);
		return $exe->result_array();
	}


	public function abl_it_get_pur_order_details($item_code){
        $this->db->where('item_code',$item_code);
        $pur_order_details = $this->db->get(db_prefix().'pur_order_detail')->result_array();		
        return $pur_order_details;
    }

	public function able_it_toget_custom_fields_items() {
		$this->db->where('fieldto','items');
		$customfields = $this->db->get(db_prefix().'customfields')->result_array();
		return $customfields;
	}
	
	//abl_it
	public function able_it_get_commodity($id = false) {
		
		if (is_numeric($id)) { 
			$this->db->where('id', $id);
			$result_array = $this->db->get(db_prefix().'items')->result_array();
	...
	//abl_it
	public function abl_it_add_goods_receipt($data, $id = false) {
