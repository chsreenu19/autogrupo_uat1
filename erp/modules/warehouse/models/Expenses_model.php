...
class Expenses_model extends App_Model
{
	...
	public function getexpensesbyinvid($id){
		
		$qry = "SELECT a.*,
		 b.name AS 'ExppenseCategory',
		 p.id AS 'ProjectId',
		...
		LEFT JOIN tblprojects p ON p.id=a.project_id
		LEFT JOIN tblclients c ON c.userid=a.clientid
		LEFT JOIN tblfiles f ON f.rel_id=a.id AND f.rel_type='expense'
		WHERE a.item_select =$id";
	...
	public function getexpensesbyinvidtotal($item_select){
		$qry = "SELECT sum(amount) as 'TotalExpenses' from ".db_prefix()."expenses where item_select =$item_select";
    ...
    public function get($id = '', $where = [])
    {
		...
		$this->db->join(db_prefix() . 'items', '' . db_prefix() . 'items.id = ' . db_prefix() . 'expenses.item_select');
        $this->db->where($where);
		//echo "<pre>"; print_r($this->db); exit;
        if (is_numeric($id)) {
			//echo 'here'; exit;
            $this->db->where(db_prefix() . 'expenses.item_select', $id);
	...
	public function updateitemselect($val, $id)
    {
		//echo $data .'----'. $id;
        $data['addexpense'] = $val;
        $this->db->where('id', $id);
		...
		//echo $this->db->last_query();
        if ($this->db->affected_rows() > 0) {
            log_activity('Add expense check Updated [ID: ' . $id . ']');
