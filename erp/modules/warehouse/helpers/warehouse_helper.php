...
function get_commodity_name_ableit($id = false)
{
	//echo $id; exit;
    $CI           = & get_instance();
    ...
    if (is_numeric($id)) {
        $CI->db->where('id', $id);
		return $CI->db->query("select * from tblitems where id=$id")->result_array();
        //return $CI->db->get(db_prefix() . 'items')->row();
    }
    if ($id == false) {
        return $CI->db->query('select * from tblitems')->result_array();
...
// ableittech check if inventory is added to any quote - 0407
function check_if_inventory_is_added_to_quote($id)
{
    $CI = & get_instance();

    $query = "SELECT a.rel_id, b.id AS 'proposal_id', b.status AS 'proposal_status' FROM ".db_prefix()."itemable a LEFT JOIN ".db_prefix()."proposals b ON a.rel_id=b.id WHERE a.inv_id=".$id;
