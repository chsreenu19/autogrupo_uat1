		...
		if (count($arr_commodity_file) > 0) {
			foreach ($arr_commodity_file as $key => $value) {
				...
				if (file_exists(WAREHOUSE_ITEM_UPLOAD . $value["rel_id"] . '/' . $value["file_name"])) {
					$images_old_value .= '<img class="image-w-h" data-dz-thumbnail alt="' . $value["file_name"] . '" src="' . site_url('modules/warehouse/uploads/item_img/' . DYNAMIC_DB.'/'. $value["rel_id"] . '/' . $value["file_name"]) . '">';
