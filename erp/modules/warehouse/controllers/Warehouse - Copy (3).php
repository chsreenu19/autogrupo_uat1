...
class warehouse extends AdminController {
	...
	function syncproducts($data){
		...
		//end of config data
			...
			$insertArray['country'] = $data['country_code'];
			...
			if($data['commodity_type']==3){
				$insertArray['regular_price'] = number_format((float)$data['rate'], 2, '.', '');
				$insertArray['sale_price'] =  number_format((float)$data['rate'], 2, '.', '');
			}else{
				$insertArray['regular_price'] = number_format((float)$data['rate'], 2, '.', '');
				$insertArray['sale_price'] =  number_format((float)$data['rate'], 2, '.', '');
			}
			...
			if(is_array($vincheck) && count($vincheck)>0){
				...
				if($data['custom_fields']['Available Status']=='Sold' || $data['custom_fields']['Available Status']=='Not for Sale'){
					...
					//$data['custom_fields']['Available Status']='For Sale';
					if($data['custom_fields']['Available Status']=='For Sale'){
	...
	function copyphotos($data,$insertId, $productId){
		...
		foreach($data as $image){
			...
			$fname = 'ableittech_'.round(microtime(true)) . '.' . end($temp);
			//echo $fname; exit;
			...
			if(file_exists($source)){
				...
				if(is_array($rarray) && count($rarray)>0){
					...
					if($rarray[0]['featured_photo']=='' || $rarray[0]['featured_photo']==null)	{
						...
						$fname = 'ableittech_'.$i.'_'.round(microtime(true)) . '.' . end($temp);
						$destination = EXPLORE_DROOT_CARS_OTHERS_PATH.$fname;
						echo $source.'==========='.$destination; 
						if( !copy($source, $destination) )
						{ 
							echo "File can't be copied! \n"; 
						}else{
							$insertArray['photo'] = $fname;
