 ...
 * warehouse controler
 */
class Invforsale extends AdminController {
	...
	public function table_commodity_list() {
		$this->app->get_table_data(module_views_path('warehouse', 'ablereports/available/table_commodity_list'));
	}
	...
	public function commodity_list($id = '') {
		...
		$data['item_groups'] = $this->warehouse_model->get_commodity_group_type();
		...
		$data['title'] = 'Inventory Available for Sale';

		$data['proposal_id'] = $id;

		$this->load->view('ablereports/available/commodity_list', $data);
	}
	...
	function syncproducts($data){
		...
		//end of config data
			...
			//echo "<pre>"; print_r($vincheck); exit;
	...
	function copyphotos($data,$insertId, $productId){
		...
		foreach($data as $image){
			...
			$fname = 'ableittech_'.$insertId.'_'.round(microtime(true)) . '.' . end($temp);
			//echo $fname; exit;
			...
			if(file_exists($source)){
				...
				if(is_array($rarray) && count($rarray)>0){
					...
					if($rarray[0]['featured_photo']=='' || $rarray[0]['featured_photo']==null)	{
						...
						$pfname = 'ableittech_'.$i.'_'.$insertId.'_'.round(microtime(true)) . '.' . end($temp);
	...
	public function ableit_view_commodity_detail_by_id($commodity_id,$newPrice) {
		...
		if(is_array($commodity_item) && count($commodity_item)>0){
			...
			$pPrice = $commodity_item['commission_pack'];
			if(!empty($newPrice)){
				if($newPrice < $pPrice){
