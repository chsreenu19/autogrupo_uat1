...
class warehouse extends AdminController {
	...
	public function commodity_list($id = '') {
		...
		$data['able_coa_accounts'] = $accounts;
		...
		$data['item_groups'] = $this->warehouse_model->get_commodity_group_type_all_ableit();
		
		$data['items_available_status'] = $this->warehouse_model->getitemavailablestatus(INV_AVAILABLE_STATUS_FIELD_ID);
		if(count($data['items_available_status'])>0){
			$avaiStatus = $data['items_available_status'][0]['options'];
			if(!empty($avaiStatus)){
				$availableStatuses = explode(",",$avaiStatus);
				$data['available_statuses'] = $availableStatuses;
			}else{
		...
		$data['all_groups'] = $this->warehouse_model->get_commodity_group_type_all_ableit();
	...
	public function able_it_commodity_customefields($id) {

		// $data = $this->db->query('select * from tblcustomfields where fieldto="items" AND ACTIVE=1')->result_array();
		$data = $this->warehouse_model->able_it_toget_custom_fields_items();
		$customfieldvalue = array();				
		if (count($data) > 0) {
			
			foreach($data as $row) {
				$customdata = 
				$this->db->query("select * from tblcustomfieldsvalues where relid=".$id." and fieldid=".$row['id']." and fieldto='items_pr'")
								->result_array();
	 ...
	 * @return view
	 */
	public function abl_it_to_get_id($id, $vendorid= false) {        
		...
		$customs_item_details = $this->able_it_commodity_customefields($id);			
		if ($customs_item_details['items_vin']) {		
			...
			if(count($pur_order_detail) == 0){
				...
				/*
				$data['order_discount'] = ''; // text box
				$data['add_discount_type']
				$data['custom_fields'] = null;  // optional
				$data['tags'] = null;  // 
		...
		if ($data) {
			...
			if (!isset($data['id'])) {
				...
				if ($ids) {
					...
					$this->abl_it_to_get_id($ids);
					
					//adding dynamic and default expenses to newly added vehicle.
					if($data['commodity_type']==1 && $data['group_id']==1){
						$this->defaultvehicleexpenses($ids);
					}	
					//end adding dynamic and default expenses to newly added vehicle.

					//ableit gl posting 
					if(IS_ITEM_GL_POSTING_ENABLED==true){
						$this->additemmappingsetup($ids,$data);
					}
	...
	public function additemmappingsetup($id,$data){
		//echo "<pre>Couint"; print_r($data); exit;
		if(isset($id) && $data>0){
			...
			if($count==0){
				$node = array();
				$node['item_id'] = $id;			
				$node['income_account'] = $data['sale_gl_account'];
				$node['expense_account'] = $data['purchase_gl_account'];
				$this->db->insert(db_prefix() . 'acc_item_automatics', $node);
				log_activity('Item Mapping created for  [VehicleID: ' . $id .  ', ClientID: ' . get_staff_user_id() . ']');
	...
	public function defaultvehicleexpenses($id){
		...
		if(is_array($expenses) && count($expenses)>0){
			foreach($expenses as $row){
				...
				if($row['exp_name']=='COMMISSION PACK'){
					$array['amount'] = COMMISSION_PACK_PRICE;
				}else{
					$array['amount'] = $row['exp_amount'];
				}
				//$array['clientid'] = '';
				//$array['project_id'] = 
				$array['currency'] = 1;
				...
				log_activity('Created expenses for vehicle [VehicleID: ' . $id . ', ExpenseID: ' . $this->db->insert_id() . ', ClientID: ' . get_staff_user_id() . ']');
			}
		}
	...
	function syncproducts($data){
		...
		if(empty($vincheck) && ($data['commodity_type']==1 || $data['commodity_type']==3)){
			...
			$bodytype = (isset($data['custom_fields']['Body Type']) && $data['custom_fields']['Body Type']!=='')? $this->ifbodytyperecordexists($data['custom_fields']['Body Type']) : '';
	...
	public function ableit_view_commodity_detail_by_id($commodity_id,$newPrice) {
		
		$commodity_item = get_commodity_name($commodity_id);
		$commodity_item = json_decode(json_encode ( $commodity_item ) , true);
		//echo "<pre>";print_r($commodity_item); exit;
		$message = '';
		$totalExpense=0;
		if(is_array($commodity_item) && count($commodity_item)>0){

			$rate = $commodity_item['rate'];
			$pPrice = $commodity_item['purchase_price'];
			if(!empty($newPrice)){
				if($newPrice < $pPrice){
					$message =  'Error, Vehicle sale price cannot be less than purchase price';
				}else{
					if($commodity_item['addexpense']==1){
						$expenses = $this->Expenses_model->getexpensesbyinvid($commodity_id);
						if(is_array($expenses) && count($expenses)>0){
							$expensesTotal = $this->Expenses_model->getexpensesbyinvidtotal($commodity_id);
							if(count($expensesTotal)>0){
					...
					$nRate = $newPrice + $totalExpense;					
					$updatePriceQry = "UPDATE ". db_prefix() ."items SET rate =".$nRate . " WHERE id=".$commodity_id;
					$this->db->query($updatePriceQry);
					if($this->db->affected_rows()>0){
						$id = get_staff_user_id();
						log_activity('Item price updated  [ItemId: ' .$commodity_id . ', Staff: ' . $id . ']');
						$message = "Success, Inventory price updated successfully";
					}else{
			...
			$message =  "Error, Invalid commodity";
		}
	...
	//update item available status ABLEIT.
	public function update_item_avaibale_status(){
		$data = $this->input->post();
		if(!empty($data)){
			if((isset($data['id']) && $data['id'] !='') && (isset($data['type']) && $data['type'] !='')){
				
				$updateItem = update_item_available_status($data['id'], $data['type']);
				//echo 'updatritem==='.$updateItem; 
				if($updateItem=='success'){
					if($data['type']=='For Sale'){
						//echo 'here'; exit;
					  $this->syncitems($data['id']);
					  echo $updateItem;
					}else{
						echo $updateItem;
					}
