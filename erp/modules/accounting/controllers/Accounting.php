    ...
    public function expenses_table()
    {
        if ($this->input->is_ajax_request()) {
            ...
            $select = [
                ...
                db_prefix() . 'expenses.item_select as inventoryId',
                //db_prefix() . 'expenses.item_select as item_select',
                db_prefix() . 'items.sku_code as item_select',
            ...
            $join         = [
                ...
                'LEFT JOIN ' . db_prefix() . 'items ON ' . db_prefix() . 'items.id = ' . db_prefix() . 'expenses.item_select',
            ...
            foreach ($rResult as $aRow) {
                ...
                //vin
                if($aRow['item_select']!=''){
                    $row[] = '<a target="blank" href="'.base_url().'admin/warehouse/view_commodity_detail/'.$aRow['inventoryId']. '#veh_expenses">'.$aRow['item_select'].'</a>';;
                }else{
                    $row[] = 'N/A';
                }    
...
//echo "<pre>"; print_r($select); exit;		
    ...
    public function journal_entry_table(){
        if ($this->input->is_ajax_request()) {
            ...
            $join = [];
            //array_push($join, ' LEFT JOIN '.db_prefix().'acc_account_history on '.db_prefix().'acc_account_history.rel_id='.db_prefix().'acc_journal_entries.id');
            ...
            foreach ($rResult as $aRow) {
                ...
                $row[] = ($aRow['ableit_control_type']!='') ? $aRow['ableit_control_type'] : 'NA';

               


                /*if($aRow['ableit_quote_id']!=''){
                    $proposalId = $aRow['ableit_quote_id'];
                    
                    $numberOutput = '<a target="blank" href="'. admin_url('proposals/list_proposals/'.$proposalId). ' " onclick=init_proposal('.$proposalId.');   >'.format_proposal_number($proposalId).'</a>';
                    $row[] =  $numberOutput;
                    
                }else{
                    $row[] = 'NA';
                }*/
                //$row[] = $aRow['ableit_quote_id'];
                if($aRow['ableit_lead_id']!='' && $aRow['ableit_lead_id']>0){
                    $leaddts = ableit_get_lead_details_by_id($aRow['ableit_lead_id']);
                    $hrefAttr = 'href="' . admin_url('leads/index/' . $aRow['ableit_lead_id']) . '" onclick="init_lead(' . $aRow['ableit_lead_id'] . ');return false;"';
                    $row[]    = '<a ' . $hrefAttr . '>' . $aRow['ableit_lead_id'] .' '.$leaddts->name.'</a>';
                ...
                if($aRow['ableit_inv_id']!='' && $aRow['ableit_inv_id']!='(NULL)'){    
                    $explode = explode("#",$aRow['ableit_inv_id']);
                              
                    $hrefAttr = 'href="' . admin_url('warehouse/view_commodity_detail/' . $explode[0]) . '" ';
                    $row[]    = '<a ' . $hrefAttr . '>' .$explode[1].'</a>';
                }else{
                    $row[] = 'NA';
                }
                
				if($aRow['ableit_type']!='' && $aRow['ableit_type']>0){
                    $staffdts = get_staff($aRow['ableit_type']);
                    $row[] = $aRow['ableit_type'];
                }else{
                    $row[] = 'NA';
                }
                
                if($aRow['ableit_sales_rep_id']!='' && $aRow['ableit_sales_rep_id']>0){
                    $staffdts = get_staff($aRow['ableit_sales_rep_id']);
                    $row[] = $staffdts->firstname.' '.$staffdts->lastname;
                }else{
                    $row[] = 'NA';
                }
    ...
    public function new_journal_entry($id = ''){
        ...
        //ableittech
        $data['proposal_ids'] = $this->accounting_model->ableit_get_proposal_ids();
        $data['proposas_sales_reps'] =  $this->accounting_model->ableit_get_sales_reps();
        $data['proposas_stocks'] =  $this->accounting_model->ableit_get_stock();
       // echo "<pre>"; print_r( $data['proposal_ids']); exit;
        //ableittech

        $jrarray = array();
        $jrarray[0]['id']='Stock';
        $jrarray[0]['label']='Stock';
        $jrarray[1]['id']='Customer';
        $jrarray[1]['label']='Customer';
        $jrarray[2]['id']='SalesRep';
        $jrarray[2]['label']='Sales Rep';
        $data['deal_types'] = $jrarray;
        $controltypes = json_decode(ACCOUNTING_CONTROL_TYPES,TRUE);
        $data['controltypes'] = $controltypes;
