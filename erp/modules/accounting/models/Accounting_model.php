...
class Accounting_model extends App_Model
{
    ...
    function get_account_data_tables($aColumns, $sIndexColumn, $sTable, $join = [], $where = [], $additionalSelect = [], $sGroupBy = '', $searchAs = [])
    {
        ...
        $this->db->select('id, number, name, parent_account, account_type_id, account_detail_type_id, balance, key_name, active, number, description, balance_as_of, '.$debit.', '.$credit.', default_account, ableit_control_type');
        if($CI->input->post('length')>0){
    ...
    //ableittech
    public function ableit_get_proposal_ids(){
        $qry = "SELECT id, proposal_to, assigned, rel_id from ".db_prefix()."proposals where (estimate_id !='' or estimate_id >0) and rel_type='lead'";
       
        $exe = $this->db->query($qry);
        $exeres = $exe->result_array();
       return $exeres;
    }

    public function ableit_get_sales_reps(){
        $qry = "SELECT id, proposal_to, rel_id,assigned from ".db_prefix()."proposals where (estimate_id !='' or estimate_id >0) and rel_type='lead'";
        $exe = $this->db->query($qry);
        $exeres = $exe->result_array();
        if(is_array($exeres) && count($exeres)>0){
            $i=0;
            foreach($exeres as $row){
                $staffdts = get_staff($row['assigned']);
                //echo "<pre>";print_r($staffdts); exit;
                $exeres[$i]['firstname'] = $staffdts->firstname;
                $exeres[$i]['lastname'] = $staffdts->lastname;
                $exeres[$i]['fullname'] = $staffdts->firstname.' '.$staffdts->lastname;
                $exeres[$i]['optionstring'] = 'ID# '.$row['assigned'].' ('.$exeres[$i]['fullname'].')';

                $i++;
            }
        }
       // echo "<pre>";print_r($exeres); exit;
       return $exeres;
    }

    public function ableit_get_stock(){
        $qry = "SELECT a.id AS 'pid', b.id AS 'itemableid', b.vin, b.inv_id
                    FROM ".db_prefix()."proposals a
                    LEFT JOIN ".db_prefix()."itemable b ON b.rel_id=a.id AND b.rel_type='proposal'
                    WHERE (a.estimate_id !='' OR a.estimate_id >0) AND a.rel_type='lead' 
                    AND (b.inv_id IS NOT NULL OR b.inv_id !='') 
                GROUP BY a.id ";
