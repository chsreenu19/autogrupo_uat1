...
$(function () {
  ...
  if (table_leads.length) {
    ...
    _table_api = initDataTable(
      ...
      LeadsServerParams,
      [table_leads.find("th.leadsortid").index(), "desc"]
    );
            ...
            .each(function (group, i) {
                if (last !== group) {
                    $(rows)
                        .eq(i)
                        .before(
                            '<tr style="background: #ccc;" class="group"><td colspan="10">Total Invoices for Deal# ' +
                                group +
                                '</td></tr>'
                        );
          ...
          if(selector==table_invoices){
            ...
            var intVal = function(i) {
                ...
                api.rows().every(function () {
                  ...
                  var groups = data[0]; // Assuming the group is in the first column
                  var tempElement = $('<div>').html(groups);
                  var anchorText = tempElement.find('a').text();
                  var group = anchorText;

                  // Calculate totals for each group
                  if (typeof total[group] === 'undefined') {
                      total[group] = 0;
                      payments[group] = 0;
                      balance[group] = 0;
                  }
                  total[group] += intVal(data[5]); // Assuming column 5 contains total
                  payments[group] += intVal(data[6]); // Assuming column 6 contains payments


                  var balspan = data[7];
                  var spanElement = $('<div>').html(balspan);
                  var spanText = spanElement.find('span').text();
                  
                  balance[group] += intVal(spanText); // Assuming column 7 contains balance with a <span>
                  console.log(balance);
                  // Insert group totals row if it's a new group
                  if (lastGroup !== group) {
                      $(rows).eq(this.index()).before('<tr style="background:#e78e094d;" class="group"><td colspan="4">' + group + '</td><td class="' + group + '-total"></td></tr>');
                      lastGroup = group;
                  }
              });
              console.log('#'+balance);
              console.log('#'+total);
              $.each(total, function (group, value) {
                var amttotal = value.toLocaleString('en-US', {
                    style: 'currency',
                    currency: 'USD'
                });
                $('.' + group + '-total').html(amttotal);
              });

              // Display payments for each group
              $.each(payments, function (group, value) {
                var pmttotal = value.toLocaleString('en-US', {
                  style: 'currency',
                  currency: 'USD'
              });
                $('.' + group + '-total').after('<td class="' + group + '-paymentstotal">' + pmttotal + '</td>');
              });

              // Display balance for each group
              $.each(balance, function (group, value) {
                var bltotal = value.toLocaleString('en-US', {
                  style: 'currency',
                  currency: 'USD'
              });
                $('.' + group + '-paymentstotal').after('<td>' + bltotal + '</td><td></td><td></td><td></td>');
              });
            ...
            api.column(0, {
              ...
              vingroup = '';
              var tempElement = $('<div>').html(group);
              var anchorText = tempElement.find('a').text();

              
              vingroup = anchorText;
                group_assoc = anchorText;
                console.log('GR#'+group_assoc);
                if (typeof total[group_assoc] != 'undefined') {
                    total[group_assoc] = total[group_assoc] + intVal(api.column(5).data()[i]);
                    payments[group_assoc] = payments[group_assoc] + intVal(api.column(6).data()[i]);
                    
                   
                    var balspan = api.column(7).data()[i];
                    var spanElement = $('<div>').html(balspan);
                    var spanText = spanElement.find('span').text();


                    console.log(spanText);
                    balance[group_assoc] = balance[group_assoc] + intVal(spanText);
                } else {
                    total[group_assoc] = intVal(api.column(5).data()[i]);
                    payments[group_assoc] = intVal(api.column(6).data()[i]);
                    var balspan = api.column(7).data()[i];
                    var spanElement = $('<div>').html(balspan);
                    var spanText = spanElement.find('span').text();
                    balance[group_assoc] = intVal(spanText);
                }
                if (last !== group) {
                    $(rows).eq(i).before(
                        '<tr style="background:#ccc" class="group"><td colspan="4">' + group + '</td><td class="' + group_assoc + '"></td></tr>'
                    );
            ...
            var pymts = $('td:eq(1)');
            pymts.after('<td>$'+payments[vingroup]+'</td>');
            console.log('#sss'+vingroup);
            var blncmats = $('td:eq(2)');
            blncmats.after('<td>$'+balance[vingroup]+'</td>');*/
...
function add_item_to_preview(id) {
  requestGetJSON("invoice_items/get_item_by_id/" + id).done(function (
	...
	//alert('here');
	  ...
	  $('input[name="invvin"]').val('');
		if (typeof (response.dynamic) !== "undefined"){
		$('.main textarea[name="description"]').val(response.description +' -vin- '+response.dynamic.VIN).attr("readonly","readonly");
	  }else{
		$('.main textarea[name="description"]').val(response.description).attr("readonly","readonly");
	  }
    //$('.main textarea[name="description"]').val(response.description +' -vin- '+response.dynamic.VIN).attr("readonly","readonly");
	...
	//$("<input type='hidden' name='invvin' id='invvin' value='"+id+"' />").insertAfter( $('form textarea[name="description"] '));
	
	  $('input[name="invvin"]').val(id);
	...
	if (typeof (response.dynamic) !== "undefined"){
		if(response.dynamic.VIN!=''){
		  $('input[name="selvehicleprice"]').val(response.rate);
		}
	  }
	...
	$('input[name="custom_fields[proposal][47]"]').val('');
	$('input[name="custom_fields[proposal][48]"]').val('');
	$('input[name="custom_fields[proposal][49]"]').val('');
	$('input[name="custom_fields[proposal][51]"]').val('');
	$('input[name="custom_fields[proposal][44]"]').val('');
	$('input[name="custom_fields[proposal][50]"]').val('');
  $('input[name="custom_fields[proposal][99]"]').val('');
  $('input[name="custom_fields[proposal][51]"]').val('');  

 if (typeof (response.dynamic) !== "undefined"){
	$('input[name="custom_fields[proposal][47]"]').val(response.dynamic['Brand']);
	$('input[name="custom_fields[proposal][48]"]').val(response.dynamic['Model']);
	$('input[name="custom_fields[proposal][49]"]').val(response.dynamic['Year']);
	$('input[name="custom_fields[proposal][44]"]').val(response.rate);
  $('input[name="custom_fields[proposal][50]"]').val(response.dynamic['Exterior Color']);
  $('input[name="custom_fields[proposal][99]"]').val(response.dynamic['Marbete']);
  $('input[name="custom_fields[proposal][51]"]').val(response.dynamic['Mileage']);  
 }
 
 if (typeof (response.dynamic) !== "undefined"){
	if (typeof response.dynamic['Marbete Vence'] !== "undefined"  && response.dynamic['Marbete Vence']!='') {
		//var dt = getFormattedDate(response.dynamic['Marbete Vence'])
		$('input[name="custom_fields[proposal][79]"]').val(response.dynamic['Marbete Vence']);  
		console.log('exists');
	}
 }
 else {
	 ...
	 if (typeof (response.dynamic) !== "undefined"){
		if (typeof response.dynamic['Marbete'] !== "undefined"  && response.dynamic['Marbete']!='') {
			//var dt = getFormattedDate(response.dynamic['Marbete Vence'])
			$('input[name="custom_fields[proposal][99]"]').val(response.dynamic['Marbete']);  
			console.log('exists');
		 }
...
function add_item_to_table(data, itemid, merge_invoice, bill_expense) {
  ...
  get_taxes_dropdown_template(tax_name, data.taxname).done(function (
    ...
    table_row +=
      '<td class="bold description"><textarea '+roStyle+' name="newitems[' +
      item_key +
    ...
    table_row +=
      '<td><textarea '+roStyle+' name="newitems[' +
      item_key +
    ...
    table_row +=
      '<td><input '+roStyle+' type="number" min="0" onblur="calculate_total();" onchange="calculate_total();" data-quantity name="newitems[' +
      item_key +
      '][qty]" value="' +
      data.qty +
      '" class="form-control">';
    ...
    table_row +=
      '<td class="rate"><input '+roStyle+' type="number" data-toggle="tooltip" title="' +
